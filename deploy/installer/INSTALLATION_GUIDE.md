# Apple Supermarket POS & ERP System — Production Installation & Administration Guide

**Document Version:** 1.0.0  
**Product Version:** 1.0.0  
**Release Tag:** `v1.0.0`  
**Vendor:** Nexora Technologies Pvt. Ltd.  
**Classification:** Confidential — for licensed customers only  
**Target Platform:** Ubuntu 24.04 LTS (x86_64)  
**Copyright:** © 2026 Nexora Technologies Pvt. Ltd.. All rights reserved.
...
| :--- | :--- |
| **Vendor Support Desk** | [support@nexoratech.example.com](mailto:support@nexoratech.example.com) |
| **Direct Phone Line** | +91-44-4000-1234 |
| **Customer Portal** | [https://www.nexoratech.example.com](https://www.nexoratech.example.com) |
| **Operating Hours** | Mon-Sat, 09:00-18:00 IST |
| **Support SLA** | First response within 1 business day; critical issues within 4 business hours. |
| **Corporate Address** | No. 12, Anna Salai, Chennai, Tamil Nadu 600002, India |
...
Apple Supermarket POS & ERP System is deployed as a hardened, containerized multi-tier on-premise application orchestrated via Docker Compose, fronted by a host-level Nginx reverse proxy providing TLS termination.
...
│                                                                                  │
│  Host-Level Nginx Reverse Proxy (/etc/nginx/sites-available/apple-pos.conf) │
...
│  ├── apple-pos.service        (Manages Docker Compose lifecycle)         │
│  └── apple-pos-backup.timer   (Nightly automated database dump at 02:00) │
└──────────────────────────────────────┬───────────────────────────────────────────┘
                                       │ 127.0.0.1:8000
                                       ▼
┌──────────────────────────────────────────────────────────────────────────────────┐
...
```bash
sudo systemctl status apple-pos-backup.timer
```
...
4. Log in using the generated administrative credentials:
   ...
   - **Password**: Displayed in the installation summary and stored in `/opt/apple-pos/.env`.
...
Apple Supermarket POS & ERP System supports multi-terminal retail setups. Satellite cashier billing terminals, barcode scanning counters, and manager tablets connect directly across the store LAN without installing software on client machines.
...
4. **Accept Self-Signed Certificate**:
   ...
   - *(Enterprise Best Practice)*: Export the root certificate from `/etc/ssl/apple-pos/fullchain.pem` on the server and deploy it to Windows/macOS **Trusted Root Certification Authorities** store across all cashier terminals.
...
2. **Username**: `admin`
3. **Password**: Displayed in installation summary or retrieved from `/opt/apple-pos/.env` (`GRAFANA_PASSWORD`).
...
- **Timer Unit**: `apple-pos-backup.timer`
- **Backup Script**: `/opt/apple-pos/deploy/installer/backup.sh`
- **Storage Location**: `/var/lib/apple-pos/backups/`
...
```bash
sudo cat /var/log/apple-pos/backup.log
```
...
2. **Stop the backend and application stack**:
   ```bash
   sudo systemctl stop apple-pos.service
   ```
...
4. **Restore the PostgreSQL database**:
   ...
   gunzip -c /tmp/posdb_live-restore.sql.gz | sudo docker exec -i apple-pos-postgres psql -U posadmin -d posdb_live
   ```
5. **Restore configuration**: Ensure `/var/lib/apple-pos/config/operation_mode.json` contains:
   ```json
   {
     "ActiveMode": "LIVE",
     "TokenVersion": 1
...
6. **Restart the application stack**:
   ```bash
   sudo systemctl start apple-pos.service
   ```
...
The installation package includes a standalone uninstaller script: [`uninstall.sh`](file:///opt/apple-pos/deploy/installer/uninstall.sh).
...
```
*Prompt: Confirms with `(y/N)`. Preserves `/var/lib/apple-pos`.*
...
1. **Interactive Slug Confirmation**: The operator must type the exact product slug (`apple-pos`) to proceed.
2. **Automated Emergency Pre-Purge Backup**: Before deleting volumes, `uninstall.sh` automatically dumps the live database to:
   `/var/backups/apple-pos-pre-purge-YYYYMMDD_HHMMSS.sql.gz`  
...
sudo docker compose -f /opt/apple-pos/docker-compose.yml ps

# View live consolidated logs
sudo docker compose -f /opt/apple-pos/docker-compose.yml logs -f --tail=100

# View backend API container logs
sudo docker logs -f apple-pos-backend
...
- **Root Cause**: The configuration file `/var/lib/apple-pos/config/operation_mode.json` is missing or unreadable (often after a volume prune or manual config directory deletion), causing the backend engine to default to UAT mode and seek `posdb_uat`.
- **Resolution**: Recreate the file with root ownership and restart the backend:
  ```bash
  sudo mkdir -p /var/lib/apple-pos/config
  sudo tee /var/lib/apple-pos/config/operation_mode.json << 'EOF'
  {
    "ActiveMode": "LIVE",
    "TokenVersion": 1
  }
  EOF
  sudo chmod 640 /var/lib/apple-pos/config/operation_mode.json
  sudo docker compose -f /opt/apple-pos/docker-compose.yml restart pos_backend
  ```
