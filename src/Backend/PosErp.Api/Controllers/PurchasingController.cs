...
public class PurchasingController : ControllerBase
{
    ...
    public async Task<IActionResult> AutoGenerateReorderPurchaseOrders([FromServices] IApplicationDbContext context)
    {
        ...
        var itemsToOrder = new List<CreatePurchaseOrderItemDto>();
        ...
        foreach (var p in lowStockProducts)
        {
            ...
            if (currentStock <= reorderThreshold)
            {
                ...
                itemsToOrder.Add(new CreatePurchaseOrderItemDto
                {
                    ProductId = p.Id,
                    Quantity = orderQty,
                    UnitPrice = p.PurchasePrice > 0 ? p.PurchasePrice : (p.SellingPrice * 0.75m)
                });
            }
        }
        ...
        if (itemsToOrder.Count > 0)
        {
            ...
            foreach (var group in itemGroups)
            {
                var poCommand = new CreatePurchaseOrderCommand
                {
                    ...
                    Notes = "Auto-Generated by AI Reorder Engine based on stock velocity & low stock thresholds",
                    Items = group
                };
