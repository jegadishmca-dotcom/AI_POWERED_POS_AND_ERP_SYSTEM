...
import { CalendarClock, Download, Calendar } from 'lucide-react';
...
export const APAging: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.get(`/api/AccountsPayable/aging?storeId=${storeId}&asOfDate=${asOfDate}`)
      .then(res => {
        setAgingData(res.data || []);
      })
      .catch(err => {
        console.error('Failed to fetch AP aging', err);
        setError('Failed to fetch AP aging report.');
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const handleExport = () => {
    ...
    const headers = ['Supplier Name', 'Total Outstanding', 'Current', '1-30 Days', '31-60 Days', '61-90 Days', '90+ Days'];
    ...
    link.setAttribute("download", `APAgingReport_${asOfDate}.csv`);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 border-b pb-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white flex items-center gap-3">
            <CalendarClock className="w-7 h-7 text-rose-600" />
            Accounts Payable (AP) Aging
          </h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1 font-semibold">Payables aging analysis (0-30, 31-60, 61-90, 90+ days)</p>
        </div>
        {agingData.length > 0 && (
          <button 
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500 font-semibold">No outstanding payables found as of this date.</div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-100 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-xs font-extrabold text-slate-500 uppercase tracking-wider">
                  ...
                  <th className="p-4 text-right bg-emerald-50/50 dark:bg-emerald-950/20">Current</th>
                  <th className="p-4 text-right bg-amber-50/50 dark:bg-amber-950/20">1 - 30 Days</th>
                  <th className="p-4 text-right bg-orange-50/50 dark:bg-orange-950/20">31 - 60 Days</th>
                  <th className="p-4 text-right bg-red-50/50 dark:bg-red-950/20">61 - 90 Days</th>
                  <th className="p-4 text-right bg-rose-50/50 dark:bg-rose-950/20">90+ Days</th>
                </tr>
              </thead>
              <tbody className="text-sm font-semibold">
                {agingData.map((e, idx) => (
                  <tr key={e.supplierId || idx} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                    <td className="p-4 font-bold text-slate-850 dark:text-slate-200">{e.supplierName}</td>
