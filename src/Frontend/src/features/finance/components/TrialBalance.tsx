...
export const TrialBalance: React.FC = () => {
  ...
  const totalDebits = data?.totalDebits ?? lines.reduce((sum: number, l: any) => sum + (Number(l.debitBalance) || 0), 0);
  const totalCredits = data?.totalCredits ?? lines.reduce((sum: number, l: any) => sum + (Number(l.creditBalance) || 0), 0);
                ...
                <>
                  {lines.map((line: any, idx: number) => (
                    <tr key={idx} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                      ...
                      <td className="p-4 text-right text-slate-600 dark:text-slate-300">{Number(line.debitBalance) > 0 ? formatCurrency(line.debitBalance) : '-'}</td>
                      <td className="p-4 text-right text-slate-600 dark:text-slate-300">{Number(line.creditBalance) > 0 ? formatCurrency(line.creditBalance) : '-'}</td>
                    </tr>
                  ))}
                  <tr className="bg-slate-50 dark:bg-slate-800/50 font-bold border-t-2 border-slate-200 dark:border-slate-700">
                    ...
                    <td className="p-4 text-right text-indigo-600 dark:text-indigo-400">{formatCurrency(totalDebits)}</td>
                    <td className="p-4 text-right text-indigo-600 dark:text-indigo-400">{formatCurrency(totalCredits)}</td>
                  </tr>
                  {totalDebits !== totalCredits && (
                    <tr>
                      <td colSpan={4} className="p-4 bg-rose-50 text-rose-600 text-center font-bold">
                        Warning: Trial Balance does not match. Difference: {formatCurrency(Math.abs(totalDebits - totalCredits))}
                      </td>
                    </tr>
                  )}
                </>
